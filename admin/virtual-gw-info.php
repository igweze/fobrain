...
/* 
	...
	-------- Script Description --------
	This script handle virtual gateway information
	------------------------------------------------------------------------*/
...
if(!session_id()){
    session_start();
		...
		try {
			...
			$virtualGatewayArr = virtualGateway($conn);  /* virtual gateways array  */
			$virtualGatewayCount = count($virtualGatewayArr);
...
?> 
		...
		<div class="table-responsive">
			...
			<table class='table table-hover table-responsive style-table wiz-table' id="wiz-table">
				...
				if($virtualGatewayCount >= $fiVal){  /* check array is empty */															
						
					for($i = $fiVal; $i <= $virtualGatewayCount; $i++){  /* loop array */	
					
						$gID = $virtualGatewayArr[$i]["gID"]; 
						$gateway = $virtualGatewayArr[$i]["gateway"];
						$gateKey = $virtualGatewayArr[$i]["gateKey"]; 
						
						$gID = trim($gID); 	
						$serial_no++; 

$virtualGateway =<<<IGWEZE
							...
							<td>	
								<div class="btn-group">
									...
									<div class="dropdown-menu dropdown-menu-lg-end p-10 dropdown-shadow end-0 animate__animated animate__flipInY"> 
										<!--
										<p class="mb-10">
											<a href='javascript:;' id='$gID' class ='view-virtual-gway text-sienna btn'>									
												<i class="mdi mdi-text-box-search label-icon"></i> View 
											</a>	
										</p>
										-->
										<p class="mb-10">
											<a href='javascript:;' id='$gID' class ='edit-virtual-gway text-slateblue btn'>									
