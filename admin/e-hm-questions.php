...
/* 
	...
	-------- Script Description --------
	This script handle student homework questions
	------------------------------------------------------------------------*/
...
if(!session_id()){
    session_start();
		...
		if ($_REQUEST['question'] == 'update') {  /* save homework question */
			...
			if ($qID == ""){
				...
				$msg_e = "* Ooops Error, please select an Home work to add Question";
				...
				if(strlen($name)) {
							...
							if (move_uploaded_file($_FILES[$filePic]['tmp_name'], $picturePath.$uploadedPic )) { /* move uploaded file to its path */
								...
								try { 
									...
									if($qID >= $fiVal){
										...
										$ebele_mark = "UPDATE $fobrainHMQuestionTB  
										...
										$ebele_mark = "INSERT INTO $fobrainHMQuestionTB  (eID, question, q1, q2, q3, q4, qPicture, ans, qMark)
									...
									if($igweze_prep->execute()){ /* insert picture name to database */
										...
										$msg_s = "Home work Question was successfully saved."; 
										echo $succesMsg.$msg_s.$sEnd; 
										echo "<script type='text/javascript'> 
											$('#homeworkQuesDiv').load('e-questions-info.php?eID=".$eID."'); 
										...
										$msg_e =  "Ooops, an error has occur while to save Home work Question. Please try again";
										echo $errorMsg.$msg_e.$eEnd; 
										echo "<script type='text/javascript'> 
					$script_scroll_cm  
				</script>"; exit;
					...
					try{
						...
						if($qID >= $fiVal){
							...
							$ebele_mark = "INSERT INTO $fobrainHMQuestionTB  (eID, question, q1, q2, q3, q4, ans, qMark)
		...
		}elseif ($_REQUEST['question'] == 'remove') {  /* remove homework question */ 
		...
		}elseif ($_REQUEST['question'] == 'view') {  /* view homework question */
			...
			if ($qID == ""){
				...
				try {
					...
					$questionInfoArr = questionHWInfo($conn, $qID);  /* online homework question information */
...
IGWEZE;
		...
		}elseif ($_REQUEST['question'] == 'edit') {  /* edit homework question */

			
			$qID = cleanInt($_REQUEST['questionID']);
			$eID = cleanInt($_REQUEST['homeworkID']);
			
			/* script validation */ 
			
			if ($eID == ""){
				
				$msg_e = "* Ooops, an error has occur while to retrieve Home work Question information. Please try again";
				echo $errorMsg.$msg_e.$eEnd; 
				echo "<script type='text/javascript'> hidePageLoader(); </script>";exit;
...
?>
					...
					<!-- form -->
					<form method="POST" class="form-horizontal" id="frmupdateQuestionHW" enctype="multipart/form-data">
						<!-- row -->
						<div class="row gutters">
							<div class="col-12">										
								...
								<div class="field-wrapper">
									...
									placeholder="Enter Home work Question, can be indent as liked "><?php echo $question; ?></textarea>
								
									<div class="field-placeholder"> Home work Question <span class="text-danger">*</span></div>
								</div>
								<!-- field wrapper end -->
							</div>									 
						</div>	
							...
							<div class="col-12 text-center">
								...
								<div class="field-wrapper">
									<label class="upload fob-btn-div">
										...
										<input type="file"id="eQuestionHWPic" name="qPicture" class="form-control ps-15 hide">
									</label> 
						...
						<div class="row gutters modal-btn-footer">
							...
							<div class="col-6 text-end">
								...
								btn-label waves-light" id="updateQuestionHW">
