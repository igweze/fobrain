...
if(!session_id()){
	...
	try {
		...
		$bankAccountDataArr = bankAccountData($conn);  /* school bank account array */
		$bankAccountDataCount = count($bankAccountDataArr);
...
?> 
        ...
        <?php
				...
				if($bankAccountDataCount >= $fiVal){  /* check array is empty */	
						
					for($i = $fiVal; $i <= $bankAccountDataCount; $i++){	
					
						$bid = $bankAccountDataArr[$i]["bid"];
						$bankAccount = $bankAccountDataArr[$i]["acc"];
						$accno = $bankAccountDataArr[$i]["accno"];
						$bank = $bankAccountDataArr[$i]["bank"];
						$balance = $bankAccountDataArr[$i]["balance"];
						$status = $bankAccountDataArr[$i]["status"];
							...
							<td> 
								<div class="btn-group">
									...
									<div class="dropdown-menu dropdown-menu-lg-end p-10 dropdown-shadow end-0 animate__animated animate__flipInY"> 
										...
										<p class="mb-10">
											<a href='javascript:;' id='$bid' class ='edit-acc-c text-slateblue btn waves-effect btn-label waves-light'>									
												<i class="mdi mdi-square-edit-outline label-icon"></i> Edit
											</a>	
										</p>
										<p>
											<a href='javascript:;' id='fobrain-$bid-$bankAccount' class ='remove-acc-c text-danger btn waves-effect btn-label waves-light'>									
