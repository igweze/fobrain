...
/* 
	...
	-------- Script Description --------
	This script handle school exam information
	------------------------------------------------------------------------*/
...
if(!session_id()){
    session_start();
		...
		if ($_REQUEST['query'] == 'save') {  /* save exam */ 
			...
			if (($eid == "")  || ($correct == "") || ($yscore == "")  || ($tscore == "")  || ($aver == "")) {
				...
				$msg_e = "* Ooops Error, could not save student exam performance";
				...
				try {		
					...
					$term = $onlineExamInfoArr[$fiVal]["eTerm"];
					$class = $onlineExamInfoArr[$fiVal]["class"];
					$eTitle = $onlineExamInfoArr[$fiVal]["eTitle"];
					$course = $onlineExamInfoArr[$fiVal]["eSubject"]; 
					$etime = $onlineExamInfoArr[$fiVal]["eTime"];
					
					$ebele_mark = "INSERT INTO $fobrainExamRevTB  (eid, regnum, course, level, class, term, etime, 
					...
					if($igweze_prep->execute()){  /* if sucessfully */ 
						...
						$msg_s = "Your exam performance was successfully saved"; 
		...
		}elseif ($_REQUEST['query'] == 'update') {  /* upttime exam */

			$rid = cleanInt($_REQUEST['rid']);	 
					
					$ebele_mark = "UPDATE $fobrainExamRevTB  
