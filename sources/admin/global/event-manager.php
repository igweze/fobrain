        ...
        if(($admin_grade == $admin_fobrain_grd)  || ($admin_grade == $hm_fobrain_grd) ){   /* check if user is admin or hm */
            ...
            if($jsonObj->request_type == 'add'){ 
                ...
                if(!empty($eventTitle)){ 
                    ...
                    $sqlQ = "INSERT INTO $notificationTB (title,comments,type,start,end) VALUES (?,?,?,?,?)"; 
                    ...
                    if($insert){ 
                        ...
                        echo json_encode(['error' => 'Ooops, Event Add request failed!']); 
                    } 
                }else{
                ...
                if(!empty($eventTitle)){ 
                    ...
                    $sqlQ = "UPDATE $notificationTB SET title=?,comments=?,type=?,start=?,end=? WHERE id=?"; 
                    $stmt = $mysqli_conn->prepare($sqlQ); 
                    ...
                    if($update){ 
                        ...
                        echo json_encode(['error' => 'Ooops, Event Update request failed!']); 
                    } 
                }else{
                    echo json_encode(['error' => 'Ooops, please enter your title']);
                } 
                ...
                $sql = "DELETE FROM $notificationTB WHERE id=$id"; 
                ...
                if($delete){ 
                    ...
                    echo json_encode(['error' => 'Ooops, Event Delete request failed!']); 
                } 
            ...
            echo json_encode(['error' => 'Ooops, only School Admin or HM can Post or Remove events']);
            exit; 
