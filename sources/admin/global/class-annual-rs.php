			...
			<thead> 
				...
				<tr> 
					<td colspan = "$top_cols"  class="text-center font-head-1  fw-600 text-info fs-13"> 
						<span class="text-danger">$level_val $class Class, $term_value</span>     
					</td>
				</tr> 
...
IGWEZE;
			...
			if(($query_i_strings_nk != "") ){				
				...
				if($rows_count_1 >= $foreal) {  /* check array is empty */ 
					...
					<th><div>Reg No. (Class)<hr class='text-danger my-5 py-0'> Student </div></th>";
					...
					for ($i = $start_nkiru; $i <= $stop_njideka; $i++) {  /* loop array */
          			...
          			echo" <th class='sort-numeric' id='scrollToTarget'><span class='vertical-lr rotated'>TASK</span></th>";
          			echo  "</tr> </thead><tbody>";
					...
					while($row[] = $igweze_prep->fetch(PDO::FETCH_BOTH)) {  /* loop array */ 
						...
						echo "<td align = 'center'> $serial_no </td>"; 
       					...
       					for ($i = $inti_reg_no_arr; $i <= $inti_reg_no_arr; $i++) {  /* loop array */
							...
							$regNum ($studentClass) <hr class='text-danger my-5 py-0' /> $stuData </span>";
							echo "</td>";
