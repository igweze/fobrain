...
/* 
	...
	-------- Script Description --------
	This script handle student staff meeting
	------------------------------------------------------------------------*/
...
if(!session_id()){
    session_start();
		...
		if ($_REQUEST['staffmeeting'] == 'save') {  /* save staff meeting */ 
			...
			if ($eTitle == "")  {
				...
				$msg_e = "* Ooops Error, please enter staff meeting duration";
				echo $errorMsg.$msg_e.$eEnd; 
				echo "<script type='text/javascript'>   hidePageLoader(); </script>";exit;
				
			}elseif ($sTime == "")  {
				
				$msg_e = "* Ooops Error, please enter staff meeting start time";
				echo $errorMsg.$msg_e.$eEnd; 
				echo "<script type='text/javascript'>   hidePageLoader(); </script>";exit;
			...
			/*elseif ($eDetail == "")  {
				...
				$msg_e = "* Ooops Error, please enter staff meeting instruction";
				echo $errorMsg.$msg_e.$eEnd; 
			...
			*/
				...
				try { 
					...
					$ebele_mark = "INSERT INTO $fobrainStaffMeetingTB  (atype, link, lpass, meetid, eType, allow, eTitle, eSubject, eTime, cTime, sTime, eDetail, eGrade, eStaff, staffs, status)

																VALUES (:atype, :link, :lpass, :meetid, :eType, :allow, :eTitle, :eSubject, 
					...
					if($igweze_prep->execute()){  /* if sucessfully */    
						...
						$msg_s = "Staff Live Meeting information was successfully saved."; 
						echo $succesMsg.$msg_s.$sEnd; 
						echo "<script type='text/javascript'> 
								$('#load-wiz-info').load('staff-meeting-info.php'); 
								//$('#frmsaveStaffMeeting').slideUp(1500); 
						...
						$msg_e =  "Ooops, an error has occur while to create new Staff Live Meeting. Please try again";
						echo $errorMsg.$msg_e.$eEnd; 
						echo "<script type='text/javascript'>   hidePageLoader(); </script>";exit;
		...
		}elseif ($_REQUEST['staffmeeting'] == 'update') {  /* update staff meeting */
			...
			if ($cid == ""){
				...
				$msg_e = "* Ooops, an error has occur to retrieve staff meeting information. Please try again";
				echo $errorMsg.$msg_e.$eEnd; 
				echo "<script type='text/javascript'> hidePageLoader(); </script>";exit;
				...
				$msg_e = "* Ooops Error, please enter staff meeting status";
				echo $errorMsg.$msg_e.$eEnd; 
				echo "<script type='text/javascript'>   hidePageLoader(); </script>";exit;
			...
			*/
				...
				*/
				...
				try { 
					...
					$ebele_mark = "UPDATE $fobrainStaffMeetingTB  
					...
					if($igweze_prep->execute()){  /* if sucessfully */
						...
						echo "<script type='text/javascript'> 
								$('#load-wiz-info').load('staff-meeting-info.php'); 
								//$('#frmupdateStaffMeeting').slideUp(1500);
								$('#modal-fobrain').modal('hide');
								hidePageLoader();  
						</script>";exit;
						...
						$msg_e =  "Ooops, an error has occur while to save Staff Live Meeting. 
						Please try again";
						echo $errorMsg.$msg_e.$eEnd; 
						echo "<script type='text/javascript'> hidePageLoader(); </script>";exit;
		...
		}elseif ($_REQUEST['staffmeeting'] == 'view') {  /* view staff meeting */ 
			...
			if ($cid == ""){
				...
				$msg_e = "* Ooops, an error has occur while to retrieve staff meeting information. Please try again";
				echo $errorMsg.$msg_e.$eEnd; 
				echo "<script type='text/javascript'> hidePageLoader(); </script>";exit;
				...
				try {
					...
					$staffMeetingInfoArr = staffMeetingInfo($conn, $cid);  /* online student staff meeting information */
						
					$atype = $staffMeetingInfoArr[$i]["atype"];
					$link = $staffMeetingInfoArr[$i]["link"];
					$lpass = $staffMeetingInfoArr[$i]["lpass"];
					$meetid = $staffMeetingInfoArr[$i]["meetid"];
					$participant = $staffMeetingInfoArr[$i]["participant"]; 
					$eType = $staffMeetingInfoArr[$i]["eType"];
					$allow = $staffMeetingInfoArr[$i]["allow"];
					$staffs = unserialize($staffMeetingInfoArr[$i]["staffs"]);
					$eStaff = $staffMeetingInfoArr[$i]["eStaff"];  
					$eTitle = $staffMeetingInfoArr[$i]["eTitle"];
					$eSubject = $staffMeetingInfoArr[$i]["eSubject"];
					$eDetail = htmlspecialchars_decode($staffMeetingInfoArr[$i]["eDetail"]);
					$eTime = $staffMeetingInfoArr[$i]["eTime"];
					$sTime = $staffMeetingInfoArr[$i]["sTime"];
					$cTime = $staffMeetingInfoArr[$i]["cTime"];
					$status = $staffMeetingInfoArr[$i]["status"];   
					...
					/*<tr><th>
						<i class="fa fa-sort-alpha-asc"></i> StaffMeeting Details </td> <td>
						$eDetail</td> </tr>
					*/	
					

$showStaffMeeting =<<<IGWEZE
...
IGWEZE;
		...
		}elseif ($_REQUEST['staffmeeting'] == 'remove') {  /* remove school staffmeeting */

			
			$staffmeeting = $_REQUEST['eData'];
			
			list($fobrainIg, $cid, $hName) = explode("-", $staffmeeting);			
			
			/* script validation */ 
			
			if (($staffmeeting == "")  || ($cid == "")){
				
				$msg_e = "* Ooops, an error has occur while to remove staff meeting. Please try again";
				echo $errorMsg.$msg_e.$eEnd; 
				echo "<script type='text/javascript'> hidePageLoader(); </script>";exit;
				...
				try { 
					...
					$ebele_mark = "DELETE FROM $fobrainStaffMeetingTB 
		...
		}elseif ($_REQUEST['staffmeeting'] == 'edit') {  /* edit staff meeting */ 
			...
			if ($cid == ""){
				...
				try {						
					...
					$staffMeetingInfoArr = staffMeetingInfo($conn, $cid);  /* online student staff meeting information */ 
					$atype = $staffMeetingInfoArr[$fiVal]["atype"];
					$link = $staffMeetingInfoArr[$fiVal]["link"];
					$lpass = $staffMeetingInfoArr[$fiVal]["lpass"];
					$eType = $staffMeetingInfoArr[$fiVal]["eType"];
					$allow = $staffMeetingInfoArr[$fiVal]["allow"];
					$staffs = unserialize($staffMeetingInfoArr[$fiVal]["staffs"]);
					$eTitle = $staffMeetingInfoArr[$fiVal]["eTitle"];
					$eSubject = $staffMeetingInfoArr[$fiVal]["eSubject"];
					$eTime = $staffMeetingInfoArr[$fiVal]["eTime"];
					$sTime = $staffMeetingInfoArr[$fiVal]["sTime"];
					$status = $staffMeetingInfoArr[$fiVal]["status"];
					//$eDetail = htmlspecialchars_decode($staffMeetingInfoArr[$fiVal]["eDetail"]);							
			
?>
					<!-- form -->
					<form class="form-horizontal" id="frmupdateStaffMeeting" role="form">
						...
						<div class="row gutters">
							<div class="col-12">										
								...
								<div class="field-wrapper select-wrapper">
									<select class="form-control fob-select"  id="staff-meet-type" name="eType" required> 
									<option value = "">Please select One</option>  
									<?php 

										foreach($stmeeting_list as $meet_key => $meet_value){  /* loop array */
									...
									<input type="hidden" value="update" name = "staffmeeting"/> 
						...
						<div class="col-lg-12" id="show_staff_div">
							<!-- field wrapper start -->
							<div class="field-wrapper">	
						...
						<div class="form-group">
							<label for="eDetail" class="col-lg-4 col-sm-4 control-label"> * StaffMeeting Instruction/s</label>
							
							<div class="col-lg-8">
							
								<textarea rows="4" cols="10" class="form-control" name="eDetail" id="eDetail" 
								placeholder="Enter StaffMeeting Instructions"><?php echo $eDetail; ?></textarea>
						...
						<div class="row gutters modal-btn-footer">
							...
							<div class="col-6 text-end">
								...
								btn-label waves-light" id="updateStaffMeeting">
		...
		}elseif ($_REQUEST['staffmeeting'] == 'start') {  /* start staff meeting */  
			...
			if ($cid == ""){
				...
				try { 
					...
					$fobrainLiveArr = staffMeetingInfo($conn, $cid);  /* online student staff meeting information */
		...
		}elseif ($_REQUEST['staffmeeting'] == 'add') {  /* start staff meeting */  

?>

			<!-- form -->
			<form class="form-horizontal mb-70" id="frmsaveStaffMeeting" role="form">	  
				<!-- row -->
				<div class="row gutters">
					<div class="col-lg-12">										
						<!-- field wrapper start -->
				...
				<div class="row gutters">
					<div class="col-12">										
						...
						<div class="field-wrapper select-wrapper">
							...
							<input type="hidden" value="save" name = "staffmeeting"/> 
							<div class="field-placeholder">  Participant <span class="text-danger">*</span></div>
						</div>
						<!-- field wrapper end -->
					</div>		 
				...
				<div id="par-meet-div">    
					...
					<div class="row gutters">
						<div class="col-12">										
							...
							<div class="field-wrapper">
								...
								<textarea rows="4" cols="10" class="form-control" name="eDetail" id="eDetail" 
								placeholder="Enter StaffMeeting Instructions"></textarea>
					...
					<div class="row gutters mt-30">
						<div class="col-12 text-end"> 
							<button type="submit" class="btn btn-primary waves-effect   
							btn-label waves-light" id="saveStaffMeeting">
								<i class="mdi mdi-content-save label-icon"></i>  Save  
							</button>
						</div>
					</div>	
