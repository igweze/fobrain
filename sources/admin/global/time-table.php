			...
			$(function(){
				...
				var calendar = new FullCalendar.Calendar(calendarEl, {
					...
					const { value: formValues } = await Swal.fire({
						title: '<i class="mdi mdi-timeline-clock-outline fs-24"></i> Add Timetable',
						html:
					...
					if (formValues) {
						...
						.then(data => {
						if (data.status == 1) {
							Swal.fire('Timetable added successfully!', '', 'success');
						} else {
					...
					Swal.fire({
						...
						.then(data => {
							if (data.status == 1) {
							Swal.fire('Timetable deleted successfully!', '', 'success'); 
							} else {
							Swal.fire(data.error, '', 'error');
							}
						...
						Swal.fire({
							title: '<i class="mdi mdi-timeline-clock-outline fs-24"></i>  Edit Timetable',						 
							html:
							...
							.then(data => {
								if (data.status == 1) {
								Swal.fire('Timetable updated successfully!', '', 'success'); 
								} else {
								Swal.fire(data.error, '', 'error');
								}
