...
?>
	...
	<script type="text/javascript"> 
		...
		$('body').on('change','#child-selector',function(){  /* select fess to pay */	
						
			showPageLoader();
			
			var select = 'children';
			var selector = $('#child-selector').val();
			
			$('#wizg-msg-box').load('selector.php', {'select': select, 'selector': selector });	 
								
			return false;  
	
		});

		$('body').on('click','.join-meeting',function(event){  /* view online liveclass */
			...
			$('#edit-msg').load( 'meeting.php', {'meeting': postVal, 'eData': liveclassID }); 
		...
		$('body').on('click','#updateRollReply',function(){  /* update online rollreply */
		
			$('#frmupdateRollReply').submit(function(event) {		
				
				showPageLoader();	
		
				event.stopImmediatePropagation();
						
				$.post('roll-call.php', $(this).find('select, input, textarea').serialize(), function(data) {
