    ...
    session_start();
        ...
        if($jsonObj->request_type == 'add'){ 
            ...
            if(!empty($eventTitle)){ 
                ...
                $sqlQ = "INSERT INTO $daily_comments_tb (title,comments,reply,start,end) VALUES (?,?,?,?,?)"; 
            ...
            if(!empty($eventTitle)){ 
                ...
                $sqlQ = "UPDATE $daily_comments_tb SET title=?,comments=?,reply=?,start=?,end=? WHERE id=?"; 
                $stmt = $mysqli_conn->prepare($sqlQ); 
        ...
        }elseif($jsonObj->request_type == 'reply'){ 
            ...
            if(!empty($eventTitle)){ 
                // Update event data into the database  
                $sqlQ = "UPDATE $daily_comments_tb SET reply=? WHERE id=?"; 
                $stmt = $mysqli_conn->prepare($sqlQ); 
                ...
                if($update){ 
                    ...
                    echo json_encode(['error' => 'Ooops, Attendance Comment Update request failed!']); 
                } 
            }else{
                echo json_encode(['error' => 'Ooops, please enter your comment']);
            } 
        }elseif($jsonObj->request_type == 'delete'){ 

            $id = $jsonObj->event_id; 
        
            $sql = "DELETE FROM $daily_comments_tb WHERE id=$id"; 
